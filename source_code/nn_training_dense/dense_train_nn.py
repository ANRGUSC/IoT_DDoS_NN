...
def create_nn_model(input_shape, output_shape):
    ...
    model.add(tf.keras.layers.Dense(4, input_shape=(input_shape,), activation='relu'))
...
def main_train_model(k_list):
    ...
    for node_index, node in enumerate(nodes):
        ...
        # in this experiment, we consider each device individually not together, so, num_labels = 1
        num_labels = 1
