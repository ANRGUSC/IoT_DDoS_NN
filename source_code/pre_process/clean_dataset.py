...
def create_benign_dataset(data, begin_date, end_date, time_step, num_nodes, benign_packet_cauchy, output_path):
    ...
    #packet = np.array(cauchy.rvs(loc=benign_packet_cauchy[0], scale=benign_packet_cauchy[1], size=benign_data.shape[0]))
    #packet = np.ceil(packet)
    #packet[packet <= 0] = 0
    #packet[packet >= benign_packet_cauchy[2]] = benign_packet_cauchy[2]
...
def main_generate_original_data():
    ...
    dataset_info_output_path = CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/dataset_info/dataset_info.csv"
    prepare_output_directory(dataset_info_output_path)
    dataset_info = load_dataset_info(CONFIG.PARKING_METER_INFO_PATH, dataset_info_output_path)

    init_data_output_path = CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/init_data/init_data.csv"
    prepare_output_directory(init_data_output_path)
    init_data = load_main_dataset(CONFIG.PARKING_METER_DATASET_PATH, init_data_output_path)

    anonym_data_map_output_path = CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/dataset_info/dataset_info_anonym.csv"
    dataset_info = create_anonymization_map(init_data, dataset_info, anonym_data_map_output_path)

    anonym_data_output_path = CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/init_data/anonym_data.csv"
    #init_data = init_data.head(100)
    init_data, anonym_data = combine_main_dataset_with_info_and_anonymize(init_data, dataset_info, init_data_output_path,
                                                 anonym_data_output_path)
    ...
    sliced_data_output_path = CONFIG.OUTPUT_DIRECTORY + "pre_process/Output/original_data/original_data_" +\
                  str(begin_date) + '_' + str(end_date) + ".csv"
